...
# Mtime: 2015/7/20 16:31
# Description:
#    处理网页请求的返回值response
#    通过beautifulsoup库分析网页
#    将有效信息提取出来，以一个课程为单位储存到字典里
#    字典们储存到列表里
#    通过信号返回给主线程
# Author: Zing
##############################################
...
class ProData(object):
    ...
    def getHadDataList(self,responce):
        soup = BeautifulSoup(responce.read(),"html.parser")
        div = soup.find(class_="grid-container")
        hadList = []
        hadNum = 0
        for tr in div.find_all(self.findTrs):
            hadList.append(dict())
            #找到名字
